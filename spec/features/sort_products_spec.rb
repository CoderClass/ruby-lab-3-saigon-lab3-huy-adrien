...
RSpec.feature "SortProducts", type: :feature do
  ...
  scenario "visit clicks on Sort by Alphabetical" do
    ...
    product3 = FactoryGirl.create(:product, name: "Henry's Hat")

    visit root_path

    # verify the products are displayed in the order they were created:
    expect(hat_links).to eq ["Brad's Hat", "Adam's Hat", "Henry's Hat"]
    ...
    expect(hat_links).to eq ["Adam's Hat", "Brad's Hat", "Henry's Hat"]
  end
end
